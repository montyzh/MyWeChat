...
(function() {
	...
	var feedback = {
		...
		geoPosition: document.getElementById('geoposition'),
		imageList: document.getElementById('image-list'),
		submitBtn: document.getElementById('submit')
	};
	
	//var url = 'http://172.20.10.4:8080/mywechat' ;
	var url = 'http://172.20.10.4:8080/mywechat/UploadHandleServlet.action';
	feedback.files = [];
	...
	feedback.clearForm = function() {
		...
		feedback.geoPosition.innerHTML = "定位";
...
//				console.log("event:"+e);
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		...
		var stateText = localStorage.getItem('$user') || "{}";
	 		var useridForString = JSON.parse(stateText).id;
	 		var userid = useridForString.toString();
	 		//console.log(JSON.parse(stateText).id);
	 		//console.log(feedback.geoPosition.innerText); 
	 		var tempPoistion = feedback.geoPosition.innerText;
//	 		
//	 		if(tempPoistion == "定位"){
//	 			
//	 			
...
//				console.log(index);
		...
		mui.alert("点击确定关闭","发布成功","确定",function () {
			feedback.clearForm();

			
			//显示朋友圈
			plus.webview.getWebviewById("friendship.html").reload(true);
			plus.webview.show("friendship.html");
		
			//有的系统自带有延时显示页面
			//隐藏当前页面时先等显示完成后再隐藏
			setTimeout(
				"plus.webview.hide('feedback.html')",
				300
			);
